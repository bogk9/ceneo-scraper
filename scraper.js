...
async function getProductStoreEntries(itemId) {
    ...
    let source = await page.goto(`https://www.ceneo.pl/${itemId.toString()}`, {
...
async function getMatchingStore(req, res) {
    const item1Id = req.query.id1;
    const item2Id = req.query.id2;
    const item3Id = req.query.id3;
    const item4Id = req.query.id4;
    ...
    results = item1Entries.filter(object => item2Entries.some(({
        url
    }) => object.url === url));
        ...
        results = results.map(item => {
            for (let items2 of item3Entries)
                if (item && (items2.url === item.url))
                    return {
                        url: item.url,
                        price: (items2.price || 0) + (item.price || 0)
                    }
        })
